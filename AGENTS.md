...
- When rebuilding the species bundle (`tools/build_species_bundle.py`), the script clears `assets/species_images/` — `dummy.webp` is explicitly preserved. If you add other hand-crafted assets to that directory, add their filenames to `_PRESERVED_IMAGES` in the script.
- To refresh the bundle: run `python tools/download_taxonomy_json.py --force` first, then `python tools/build_species_bundle.py`.

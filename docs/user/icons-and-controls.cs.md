# Ikony a ovládací prvky

Tato stránka vysvětluje opakující se ovládací prvky a symboly používané v BirdNET Live. Štítky níže odpovídají ovládacím prvkům, které již v aplikaci jsou.

## Sdílené ovládací prvky navigace

| Ovládání | Kde to vidíš | Co to dělá |
|---|---|---|
| :material-tune: **Nastavení** | Home footer, Live, Point Count, Survey, File Analysis, Session Review | Otevře Nastavení. Na obrazovkách režimu otevře nastavení, která jsou pro daný pracovní postup nejrelevantnější. |
| :material-zvětšit: **Prozkoumat** | Zápatí domů | Otevře Prozkoumat. |
| :material-music-box-multiple-outline: **Knihovna** | Zápatí domů | Otevře knihovnu relací. |
| :material-help-circle-outline: **Nápověda** | Zápatí domovské stránky, záhlaví prozkoumat, panel průzkumu, panel nástrojů Kontrola relace | Otevře nápovědu nebo list nápovědy pro konkrétní obrazovku. |
| :informace-materiálu: **Info / O** | Domovská zápatí, informační lišty, pomocné listy | Zobrazuje obecné informace nebo souhrnný kontext. |
| :material-arrow-left: **Zpět** | Živý režim | Vrátí se na předchozí obrazovku. |
| :material-open-in-new: **Otevřít externí** | O obrazovce, odkazy na dokumentaci | Otevře externí stránku, jako je online Uživatelská příručka. |
| :material-volunteer-activism: **Přispět** | Obrazovka O aplikaci | Otevře stránku pro příspěvky BirdNETu. |

## Symboly počasí

| Symbol | Význam |
|---|---|
| :material-wb-sunny: **Jasno** | Jasná obloha. |
| :material-partly-cloudy-day: **Polojasno** | Slunce a mrak pro převážně jasné nebo polojasné počasí. |
| :material-cloudy: **Zataženo** | Souvislá oblačnost. |
| :material-foggy: **Mlha** | Mlha nebo námrazová mlha. |
| :material-rainy-light: **Mrholení** | Lehké srážky. |
| :material-rainy: **Déšť** | Déšť nebo dešťové přeháňky. |
| :material-weather-snowy: **Sníh** | Sníh nebo sněhové přeháňky. |
| :material-thunderstorm: **Bouřka** | Bouřkové podmínky. |

## Ovládací prvky pro spuštění, zastavení a relace

| Ovládání | Význam |
|---|---|
| :materiál-mikrofon: **Mikrofon** | Spusťte živý poslech. |
| :material-stop: **Stop** | Zastavte aktivní nahrávání, počítání bodů nebo průzkum. |
| :material-play: **Hrát** | Spusťte nakonfigurovaný tok nastavení nebo pokračujte ze stavu pozastaveno-připraven. |
| :material-close: **Zavřít / Zrušit** | Zrušit aktivní analýzu souborů. |
| :material-timer: **Časovač** | Trvání nebo zbývající čas. |
| :material-alert-circle-outline: **Chyba** | Chyba modelu nebo zpracování. |

## Ovládání polohy a času

| Ovládání | Význam |
|---|---|
| :material-crosshairs-gps: **Aktuální poloha** | Použijte aktuální polohu GPS zařízení. |
| :material-map-marker-plus: **Manuální souřadnice** | Zadejte souřadnice ručně. |
| :material-map-marker-off: **Žádné umístění** | Přeskočte polohu nebo ukažte, že poloha není k dispozici. |
| :material-map-marker: **Má polohu** | Potvrďte polohu, zobrazte souřadnice nebo označte mapovanou relaci. |
| :material-refresh: **Refresh** | Znovu si přečtěte aktuální polohu nebo obnovte seznam předpovědí. |
| :material-map: **Výběr mapy** | Vyberte souřadnice z nástroje pro výběr mapy. |
| :material-calendar: **Datum** | Nastavte nebo zobrazte datum. |
| :material-close: **Jasné** | Odebrat vybrané datum. |

## Prozkoumejte a druhy symbolů

| Ovládání | Význam |
|---|---|
| Miniatura druhu | Přiložený obrázek pro daný druh, pokud je k dispozici. |
| Procentuální odznak spolehlivosti nebo geomodelu | Rychlé číselné shrnutí výstupu modelu. Vyšší čísla znamenají silnější podporu v kontextu dané obrazovky. |
| Měsíční štítky (`led`, `duben`, `červenec`, `říjen`, ,prosinec`) | Referenční body na týdenním grafu očekávané frekvence v druhovém překrytí. |

## Panel nástrojů pro kontrolu relace

Tyto ovládací prvky se používají na obrazovce Session Review.

| Ovládání | Význam |
|---|---|
| :material-plus-circle-outline: **Přidat** | Přidejte obsah, jako je druh nebo anotace. |
| :material-undo-variant: **Undo** / :material-redo-variant: **Redo** | Krok zpět nebo vpřed v úpravách recenzí. |
| :material-content-cut: **Trim** | Vstupte do režimu trimování nebo ukažte, že režim trimování je aktivní. |
| :material-content-save: **Uložit** | Uložit změny recenze. |
| :material-share-varianta: **Sdílet** | Exportujte nebo sdílejte relaci. |
| :material-delete-outline: **Smazat** | Zrušte relaci. |
| :material-play: **Pokračovat** | Až bude tato akce k dispozici, pokračujte v nedokončeném průzkumu z Kontrola relace. |

## Stavové pruhy specifické pro obrazovku

### Živý režim

Aktuální informační panel používá :material-information-outline: následovaný kompaktními štítky, jako jsou:

- `now` — detekce aktuálně viditelné v živém seznamu
- `spp` — počet unikátních druhů
- `det` — celkový počet detekcí
- trvání a odhadovaná velikost záznamu, když je záznam aktivní

### Počet bodů

Ukazatel časovače počtu bodů kombinuje :material-stop: **Stop**, :material-timer: **Timer** a ukazatel průběhu pro zobrazení zbývající měřené relace.

### Průzkum

Panel průzkumu používá:

- :material-map-outline: **Mapa** — karta živá mapa
- :material-ekvalizér: **Spektrogram** — záložka spektrogram
- :material-chart-bar: **Souhrn** — karta shrnutí
- :material-chart-bar: statistiky štítků v zobrazení souhrnu průzkumu

## Když máte pochybnosti

Pokud si nejste jisti, co ovládací prvek dělá, otevřete nejbližší list nápovědy v aplikaci nebo se podívejte na stránku pracovního postupu pro danou obrazovku v této příručce k používání.
